          ...
          <div className="border-b border-border bg-bg-alt p-5 md:border-b-0 md:border-r">
            ...
            <div className="grid min-h-[184px] place-items-center rounded-[8px] border border-dashed border-border-strong bg-bg">
              <div className="text-center">
                ...
                <p className="mb-0 mt-3 max-w-[180px] truncate text-sm font-semibold">sample-interview.mp4</p>
                <p className="mb-0 mt-1 font-mono text-[10px] text-fg-3">{locale === 'zh' ? '示例' : 'SAMPLE'} · 02:18 · MP4</p>
              </div>
            </div>
    ...
    <section className="grid-bg grid items-center gap-12 overflow-hidden px-5 py-14 md:grid-cols-[.88fr_1.12fr] md:px-7 md:py-24">
      <div className="mx-auto flex w-full max-w-[560px] flex-col gap-[18px]">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-3">
          {loggedIn
            ? <Link to="/{-$locale}/app/render" onClick={() => trackMarketingCta({ ctaId: 'hero_render', placement: 'hero', locale, destination: '/app/render' })} className={buttonVariants({ size: 'lg' })}>{t('marketing.heroCtaPrimary')} <ArrowRight size={18} /></Link>
            : <Link to="/{-$locale}/register" search={{ next: '/app/render' }} onClick={() => trackMarketingCta({ ctaId: 'hero_register', placement: 'hero', locale, destination: '/register' })} className={buttonVariants({ size: 'lg' })}>{t('marketing.heroCtaPrimary')} <ArrowRight size={18} /></Link>}
