...
配置自定义域后,生产环境会关闭 `workers.dev`,避免用户从非 canonical Origin 进入认证流程。
`CF_PROD_DOMAIN` 会同时生成裸域 Custom Domain 和 `www.<domain>/*` route;Worker 将 `www`
永久重定向到 `BETTER_AUTH_URL`,并保留路径与查询参数。
...
- `deploy.yml`——跑 `.github/scripts/gen-wrangler.mjs`,把真实生产 id 注入 production 段。
  ...
  | `CF_PROD_DOMAIN` | 可选 | canonical 裸域(如 `example.com`),设了会注入裸域和 `www` 路由 |
