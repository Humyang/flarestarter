...
function jobRoute(pathname: string): { id: string; action: 'status' | 'output' } | null {
  const match = pathname.match(/^\/api\/render-agent\/jobs\/([^/]+)\/(status|output)$/)
  if (!match) return null
  try {
    return { id: decodeURIComponent(match[1]!), action: match[2]! as 'status' | 'output' }
  } catch {
    return null
  }
}
